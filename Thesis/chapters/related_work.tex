...
The API Gateway is responsible for edge security, therefore it is the only entry point to the microservice deployment.
...
Since the microservices communicate with each other using remote calls, their communication has to provide mutual authentication~\cite{dias2020microservices}.
...
\begin{description}
	...
	\item[signatureAlogrithm:] declares the cryptographic algorithm that was used to sign the certificate.
...
The record protocol provides a secure channel for the communication of two parties.
...
Both the server and the client must own a private/public key pair.
